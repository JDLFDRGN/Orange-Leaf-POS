...
<?php
    ...
    $account = $_SESSION['account'];
     ...
     if(isset($_POST['edit-employee-submit'])){
        ...
        $admin->adminUpdateEmployeeProfile($account['employee_id'], $profile, $firstname, $lastname, $email, $gender, $number, $address);
        $staff->employeeUpdateSession($account['employee_id']);
...
<body>
    ...
    <section class="min-height-full w-full">
       <form class="w-3/4 mx-auto p-4" style="margin-top: 5em;"  method="post" enctype="multipart/form-data">
            <h1 class="uppercase bold font-bold text-2xl link">edit employee</h1>
            <div class="flex">
            ...
            <div class="mt-32 flex justify-between items-center">
                <a class="underline link" href="employee-update-mypassword.php">Change password</a>
                <div>
                    <input class="button1 w-32 py-2 rounded-lg mr-4 font-bold" type="button" value="Cancel" onclick="navigate('employee-home.php')"> 
                    <input class="button1 w-32 py-2 rounded-lg font-bold" type="submit" value="Submit" name="edit-employee-submit">
                </div>
            </div>
       </form>
    </section>
