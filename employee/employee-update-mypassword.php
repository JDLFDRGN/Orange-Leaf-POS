...
<?php
   include '../sql/queries.php';
    ...
    if(isset($_POST['employee-password-submit'])){
        ...
        if($newPassword == $retypeNewPassword){
            if($oldPassword == $account['password']){
                $staff->employeeUpdatePassword($account['employee_id'], $newPassword);
                echo "<script>alert('Password updated successfully'); window.location = 'employee-home.php';</script>";
            }else{
        ...
        ?>
         <div class="w-full flex justify-center">
            <form class="container border-show rounded-lg p-4 m-20 flex flex-col items-center w-2/5" method="post">
                ...
                <input class="button1 font-bold w-28 rounded-lg py-1 mt-10" type="submit" value="Update" name="employee-password-submit">
                <a class="mt-4 underline capitalize link" href="employee-my-account.php">back</a>
            </form>
        </div>
    </section>
